        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <form action="{{ route('home') }}" method="GET" id="filterForm">
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-lg overflow-hidden"
                    x-data="{ open: {{ request()->hasAny(['search', 'min_price', 'max_price', 'platform', 'publisher']) ? 'true' : 'false' }} }">

                    {{-- Header toggle --}}
                    <button type="button" @click="open = !open"
                        class="w-full px-6 py-4 bg-gradient-to-r from-miku-500 to-miku-600 hover:from-miku-600 hover:to-miku-700 text-white font-black text-lg flex items-center justify-between transition shadow-md">
                    ...
                    <div x-show="open" x-transition class="p-6 border-t border-gray-200 dark:border-gray-700">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div>
                                <label
                                    class="text-xs font-bold text-gray-600 dark:text-gray-400 uppercase tracking-wide block mb-2">💰
                                    Từ giá</label>
                                <input type="number" name="min_price" placeholder="Từ"
                                    value="{{ request('min_price') }}"
                                    class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg focus:ring-0 focus:border-orange-500 outline-none transition text-sm">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="platform[]" multiple
                                    class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg focus:ring-0 focus:border-blue-500 outline-none transition text-sm">
                                    <option value="">Chọn...</option>
                                    @foreach (['PC', 'PS5', 'Xbox Series X/S', 'Nintendo Switch'] as $platform)
                                        <option value="{{ $platform }}"
                                            {{ in_array($platform, (array) request('platform', [])) ? 'selected' : '' }}>
                                            {{ $platform }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            @if ($publishers->count() > 0)
                                <div>
                                    ...
                                    <select name="publisher"
                                        class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg focus:ring-0 focus:border-purple-500 outline-none transition text-sm">
                                        <option value="">Tất cả</option>
                                        @foreach ($publishers as $pub)
                                            <option value="{{ $pub }}"
                                                {{ request('publisher') === $pub ? 'selected' : '' }}>
                                                {{ substr($pub, 0, 15) }}{{ strlen($pub) > 15 ? '...' : '' }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            @endif
                        ...
                        <div class="flex gap-3 mt-6">
                            <button type="submit"
                                class="flex-1 bg-gradient-to-r from-miku-500 to-miku-600 hover:from-miku-600 hover:to-miku-700 text-white font-bold py-2 px-4 rounded-lg transition shadow-md transform hover:scale-105 duration-300">
                                <i class="fa-solid fa-check mr-2"></i>Áp dụng
                            </button>
                            @if (request()->hasAny(['search', 'category', 'min_price', 'max_price', 'platform', 'publisher']))
                                <a href="{{ route('home') }}"
                                    class="flex-1 bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-lg transition shadow-md transform hover:scale-105 duration-300 flex items-center justify-center gap-2">
            ...
            <form action="{{ route('home') }}" method="GET" class="mb-6">
                <div class="flex gap-2 flex-wrap">
                    <button type="submit" name="category" value=""
                        class="px-4 py-2 rounded-lg font-semibold text-sm transition {{ !request('category') ? 'bg-miku-500 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 hover:border-miku-500' }}">
                        ⭐ Tất cả
                    </button>
                    @foreach ($categories as $category)
                        <button type="submit" name="category" value="{{ $category->slug }}"
                            class="px-4 py-2 rounded-lg font-semibold text-sm transition {{ request('category') === $category->slug ? 'bg-miku-500 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700 hover:border-miku-500' }}">
                            {{ $category->name }}
                        </button>
                    @endforeach
                </div>
